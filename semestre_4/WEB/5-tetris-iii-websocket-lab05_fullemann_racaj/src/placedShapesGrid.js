export class PlacedShapesGrid {
  ...
  getBlocksPerPlayer() {
    ...
    for (let row = 0; row < this.height; row++) {
      for (let col = 0; col < this.width; col++) {
        const playerId = this.map[row][col];
        if (playerId !== -1) {
          const count = blocksCount.get(playerId) || 0;
          blocksCount.set(playerId, count + 1);
        }
      }
    }
  ...
  clear() {
    ...
    // Only update UI elements if in browser environment
    if (typeof document !== "undefined") {
      const curr_player = document.getElementById("currentPlayer");
      if (curr_player && this.playerId !== undefined) {
