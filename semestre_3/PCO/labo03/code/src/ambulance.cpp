...
void Ambulance::sendPatient() {
    if (getNumberPatients() < 1) return;

    if (chooseRandomSeller(hospitals)->send(ItemType::PatientSick, 1, getCostPerUnit(ItemType::PatientSick))) {
        stocks[ItemType::PatientSick]--;
        nbTransfer++;
        money += getCostPerUnit(ItemType::PatientSick);
        interface->consoleAppendText(uniqueId, "Patient sent");
    }
}
