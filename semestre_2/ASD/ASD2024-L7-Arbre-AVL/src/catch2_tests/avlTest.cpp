...
TEST_CASE("min", "[avlTree]") {
    SECTION("Throws if empty") {
        avlTree<int> tree;
        REQUIRE_THROWS_MATCHES(tree.min(), std::invalid_argument, Catch::Matchers::Message("Empty tree"));
    }
...
TEST_CASE("max", "[avlTree]") {
    SECTION("Throws if empty") {
        avlTree<int> tree;
        REQUIRE_THROWS_MATCHES(tree.max(), std::invalid_argument, Catch::Matchers::Message("Empty tree"));
    }
...
TEST_CASE("erase_min", "[avlTree]") {
    SECTION("Throws if empty") {
        avlTree<int> tree;
        REQUIRE_THROWS_MATCHES(tree.erase_min(), std::invalid_argument,Catch::Matchers::Message("Empty tree"));
    }
...
TEST_CASE("erase_max", "[avlTree]") {
    SECTION("Throws if empty") {
        avlTree<int> tree;
        REQUIRE_THROWS_MATCHES(tree.erase_max(), std::invalid_argument, Catch::Matchers::Message("Empty tree"));
    }
...
TEST_CASE("operator>>", "[avlTree]") {
    ...
    SECTION("Read tree with values") {
        std::istringstream iss("6(3(2(1(.,.),.),4(.,5(.,.))),9(7(.,8(.,.)),10(.,.)))");
        avlTree<int> tree;
        iss >> tree;
        REQUIRE(tree.height() == 4);
    }
...
TEST_CASE("Initializer list constructor", "[avlTree]") {
    SECTION("Construct tree with initializer list") {
        avlTree<int> tree = {5, 3, 7, 2, 4, 6, 8};
        REQUIRE(tree.height() == 3);
    }
}
