...
impl Authorizer {
    pub fn new(pubkey: PublicKey) -> Self {
       let policy = authorizer!(r#"
            allow if user($u), patient($u), operation("read-patient");
            allow if user($u), patient($u), operation("write-patient");

            allow if user($u), patient($u), operation("read-report");

            allow if user($u), author($u), operation("read-report");

            allow if user($u), is_doctor($u, true), operation("read-patient");

            allow if user($u), is_doctor($u, true), author($u), operation("create-report");
            "#);
